...
void RPU_PlaySoundDash32(byte soundByte, bool address_E) {
    ...
    // If out of index --> play silence
    if(soundByte > 15) soundByte = 15;
/*
    if(!address_E) {
        if(soundByte == 0) soundByte = 1;
        else if(soundByte == 1) soundByte = 0;
    }
*/

    // PIA Port B
    byte oldSolenoidControlByte = RPU_DataRead(ADDRESS_U11_B);
    byte soundLowerNibble = (oldSolenoidControlByte & 0xF0) | (soundByte & 0x0F);

    // PIA Port A
    byte oldDisplayByte = RPU_DataRead(ADDRESS_U11_A);
    byte displayWithSoundBit4 = oldDisplayByte;

    // mask further zero-crossing interrupts during this
    noInterrupts();

    if(address_E) displayWithSoundBit4 &= 0xFD;
    else displayWithSoundBit4 |= 0x02;
